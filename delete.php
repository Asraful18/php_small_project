...
<?php
 if(isset($_POST['submit'])){
    ...
    if($_POST['selected']=="Name"){
      $s="SELECT * FROM stdunt1 WHERE name='$input'";
      $row=mysqli_query($con,$s);
      $s1="DELETE FROM stdunt1 WHERE name='$input'";
      $row1=mysqli_query($con,$s1);
    }
    if($_POST['selected']=="Mobile No."){
      $s="SELECT * FROM stdunt1 WHERE mobile='$input'";
      $row=mysqli_query($con,$s);
      $s1="DELETE FROM stdunt1 WHERE mobile='$input'";
      $row1=mysqli_query($con,$s1);
    }
    if($_POST['selected']=="All"){
      $s="SELECT * FROM stdunt1 ";
      $row=mysqli_query($con,$s);
      $s1="DELETE FROM stdunt1";
      $row1=mysqli_query($con,$s1);
    }
}
?>
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link" aria-current="page" href="display.php">Display</a>
        </li>
      </ul>
    </div>
  </div>
...
<div class="container px-5 my-5">
    <form class="d-flex justify-content-center text-aligne-center" id="contactForm" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="post">
        <select class="form-select w-50 h-25 mx-5 bg-danger text-light" aria-label="Default select example" name="selected">
...
<div class="card w-50" style="margin-left: 25%;">
  <div class="card-header bg-danger text-light">
   <?php   
    echo mysqli_num_rows($row)." rows is deleted successfully";?>
  </div>
